...
type Tab = 'dashboard' | 'movements' | 'cards' | 'calendar' | 'more' | 'analysis' | 'goals' | 'settings'
...
function App() {
  ...
  const toggleMovementPaid = (id: string) => {
    setState((previous) => ({
      ...previous,
      movements: previous.movements.map((movement) => movement.id === id ? { ...movement, paid: !movement.paid } : movement),
    }))
  }
    ...
    try {
      ...
      for (let offset = 0; offset < 3; offset += 1) {
        ...
        for (const item of calendarItems(state, targetMonth)) {
          if (item.kind !== 'expense' || item.amount <= 0) continue
  ...
  const closeMonth = () => {
    ...
    const record = {
      ...
      paidMovements: state.movements.filter((m) => m.paid && m.type === 'expense' && m.date.startsWith(month)).reduce((sum, m) => sum + m.amount, 0),
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {tab === 'dashboard' && (
          <Dashboard state={state} month={month} snapshot={snapshot} health={health} dailySafe={dailySafe}
            onAdd={() => setShowMovement(true)} onCards={() => setTab('cards')} onGoals={() => setTab('goals')} onCalendar={() => setTab('calendar')} />
        )}
        {tab === 'movements' && <Movements state={state} month={month} onAdd={() => setShowMovement(true)} onToggle={toggleMovementPaid} onRemove={removeMovement} />}
        {tab === 'cards' && <Cards state={state} month={month} setState={setState} updateConfig={updateConfig} onAdd={() => setShowPurchase(true)} />}
        {tab === 'calendar' && <CalendarView state={state} month={month} onToggle={toggleMovementPaid} />}
        ...
        {tab === 'settings' && (
          <SettingsPanel state={state} setState={setState} updateConfig={updateConfig} updateText={updateText} biometryAvailable={biometryAvailable}
            onCheckUpdate={checkUpdate} onExport={exportPlainBackup} onSecureBackup={shareEncryptedBackup} onImport={() => importRef.current?.click()}
            onSyncNotifications={syncNotifications} onLock={() => setLocked(true)} />
        )}
      </main>
      ...
      <nav className="bottom-nav">
        ...
        <NavButton active={['more', 'analysis', 'goals', 'settings'].includes(tab)} label="Mais" icon={<MoreHorizontal />} onClick={() => setTab('more')} />
      </nav>
    </div>
  )
}
...
function navTitle(tab: Tab) {
  const titles: Record<Tab, string> = { dashboard: 'Meu mês', movements: 'Lançamentos', cards: 'Cartões', calendar: 'Agenda financeira', more: 'Organização', analysis: 'Análises', goals: 'Metas e reservas', settings: 'Ajustes' }
  return titles[tab]
}
...
function Dashboard({ state, month, snapshot, health, dailySafe, onAdd, onCards, onGoals, onCalendar }: { state: AppState; month: string; snapshot: ReturnType<typeof monthSnapshot>; health: string; dailySafe: number; onAdd: () => void; onCards: () => void; onGoals: () => void; onCalendar: () => void }) {
  const items = calendarItems(state, month).filter((item) => item.date >= todayIso()).slice(0, 4)
  const cardNext = cardInvoiceForMonth(state, month)
  ...
  return <section className="stack">
    <article className={`hero-card ${health}`}>
      <div className="hero-label"><CircleDollarSign size={18} /><span>Sobra sem depender de renda variável</span></div>
      <strong className="hero-value">{money(snapshot.guaranteedMargin)}</strong>
      <p>{health === 'attention' ? 'Seu orçamento garantido não fecha sozinho neste cenário.' : health === 'tight' ? 'O mês fecha, mas a margem para imprevistos está curta.' : 'Seu orçamento garantido mantém uma margem positiva.'}</p>
      <div className="range-row"><span>Com renda extra variável</span><b>{money(snapshot.lowMargin)} a {money(snapshot.highMargin)}</b></div>
    </article>

    <div className="grid-two">
      <Metric label="Renda garantida" value={money(snapshot.guaranteedIncome)} hint="salário + entradas fixadas" />
      <Metric label="Comprometido" value={`${commitment.toFixed(0)}%`} hint={money(snapshot.plannedExpenses)} />
      <Metric label="Margem por dia" value={money(dailySafe)} hint="sem usar renda extra" />
      <Metric label="Cartão no mês" value={money(snapshot.fixed.card)} hint={state.config.cardMode === 'actual' ? 'fatura calculada' : 'orçamento definido'} />
    </div>

    {snapshot.guaranteedMargin < 0 && <article className="alert-card"><AlertTriangle size={20} /><div><b>Atenção ao teto de gastos</b><p>Faltam {money(Math.abs(snapshot.guaranteedMargin))} para o salário garantido cobrir tudo. Evite assumir novas parcelas fixas até reequilibrar.</p></div></article>}

    <div className="quick-actions">
      <button onClick={onAdd}><Plus />Lançar</button><button onClick={onCards}><CreditCard />Cartão</button><button onClick={onCalendar}><CalendarDays />Agenda</button><button onClick={onGoals}><Target />Metas</button>
    </div>

    <article className="section-card">
      <div className="section-heading"><div><span className="section-kicker">PRÓXIMOS</span><h2>Compromissos</h2></div></div>
      {items.length ? <div className="compact-list">{items.map((item) => <div className="compact-row" key={`${item.id}-${item.date}`}><div><b>{item.title}</b><small>{dateLabel(item.date)}</small></div><strong className={item.kind}>{item.kind === 'income' ? '+' : '-'} {money(item.amount)}</strong></div>)}</div> : <Empty text="Nenhum compromisso cadastrado para frente neste mês." />}
    </article>

    <article className="section-card">
      <div className="section-heading"><div><span className="section-kicker">BASE ATUAL</span><h2>Orçamento essencial</h2></div><b>{money(snapshot.fixed.fixed)}</b></div>
      <div className="budget-lines">{Object.entries(snapshot.fixed.categories).map(([name, value]) => <div key={name}><span>{name}</span><b>{money(value)}</b></div>)}</div>
      {state.config.cardMode === 'actual' && cardNext === 0 && <small className="muted">Sem compras cadastradas no cartão para este mês.</small>}
    </article>
  </section>
}
...
function Movements({ state, month, onAdd, onToggle, onRemove }: { state: AppState; month: string; onAdd: () => void; onToggle: (id: string) => void; onRemove: (id: string) => void }) {
  ...
  return <section className="stack">
    <div className="grid-two"><Metric label="Entradas adicionais" value={money(income)} /><Metric label="Saídas adicionais" value={money(expense)} /></div>
    <button className="primary full" onClick={onAdd}><Plus size={18} /> Novo lançamento</button>
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">MÊS</span><h2>Movimentações</h2></div><b>{movements.length}</b></div>
      {movements.length ? <div className="movement-list">{movements.map((movement) => <div className={`movement-row ${movement.paid ? 'paid' : ''}`} key={movement.id}>
        <button className="status-check" onClick={() => onToggle(movement.id)}>{movement.paid ? <CheckCircle2 size={22} /> : <span />}</button>
        <div className="movement-copy"><b>{movement.description}</b><small>{movement.category} • {dateLabel(movement.date)}{movement.recurrence === 'monthly' ? ' • mensal' : ''}</small></div>
        <strong className={movement.type}>{movement.type === 'income' ? '+' : '-'} {money(movement.amount)}</strong>
        <button className="icon-button danger" onClick={() => onRemove(movement.id)}><Trash2 size={17} /></button>
      </div>)}</div> : <Empty text="Nenhum lançamento adicional neste mês." />}
    </article>
  </section>
}
...
function Cards({ state, month, setState, updateConfig, onAdd }: { state: AppState; month: string; setState: React.Dispatch<React.SetStateAction<AppState>>; updateConfig: <K extends keyof BudgetConfig>(key: K, value: BudgetConfig[K]) => void; onAdd: () => void }) {
  ...
  return <section className="stack">
    ...
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">MODO</span><h2>Como considerar cartão</h2></div></div>
      <div className="segmented"><button className={state.config.cardMode === 'budget' ? 'active' : ''} onClick={() => updateConfig('cardMode', 'budget')}>Orçamento fixo</button><button className={state.config.cardMode === 'actual' ? 'active' : ''} onClick={() => updateConfig('cardMode', 'actual')}>Fatura real</button></div>
      {state.config.cardMode === 'budget' && <MoneyField label="Orçamento mensal do cartão" value={state.config.cardBudget} onChange={(v) => updateConfig('cardBudget', v)} />}
    </article>

    {state.cards.map((card) => { const invoice = cardInvoiceForMonth(state, month, card.id); const committed = cardCommittedLimit(state, card.id); const available = card.limit > 0 ? Math.max(0, card.limit - committed) : 0; return <article className="section-card" key={card.id}>
      <div className="section-heading"><div><span className="section-kicker">CARTÃO</span><h2>{card.name}</h2></div><b>{money(invoice)}</b></div>
      <div className="form-grid two"><TextField label="Nome" value={card.name} onChange={(v) => updateCard(card.id, { name: v })} /><MoneyField label="Limite" value={card.limit} onChange={(v) => updateCard(card.id, { limit: v })} /><NumberField label="Fecha dia" value={card.closingDay} onChange={(v) => updateCard(card.id, { closingDay: v })} /><NumberField label="Vence dia" value={card.dueDay} onChange={(v) => updateCard(card.id, { dueDay: v })} /></div>
      {card.limit > 0 && <div className="limit-box"><span>Comprometido em parcelas</span><b>{money(committed)}</b><small>Limite estimado disponível: {money(available)}</small><div className="progress"><i style={{ width: `${Math.min(100, (committed / card.limit) * 100)}%` }} /></div></div>}
    </article> })}
    <button className="primary full" onClick={onAdd}><Plus size={18} /> Registrar compra no cartão</button>
    ...
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">COMPRAS</span><h2>Parcelas e recorrências</h2></div><b>{state.cardPurchases.filter((p) => p.active).length}</b></div>
      {state.cardPurchases.length ? <div className="movement-list">{state.cardPurchases.map((purchase) => <div className="movement-row" key={purchase.id}><div className="movement-copy"><b>{purchase.description}</b><small>{purchase.recurring ? 'recorrente mensal' : `${purchase.installments}x de ${money(purchase.totalAmount / Math.max(1, purchase.installments))}`} • {dateLabel(purchase.purchaseDate)}</small></div><strong>{money(purchase.totalAmount)}</strong><button className="icon-button danger" onClick={() => setState((p) => ({ ...p, cardPurchases: p.cardPurchases.filter((item) => item.id !== purchase.id) }))}><Trash2 size={17} /></button></div>)}</div> : <Empty text="Nenhuma compra cadastrada." />}
    </article>
  </section>
}
...
function CalendarView({ state, month, onToggle }: { state: AppState; month: string; onToggle: (id: string) => void }) {
  ...
  return <section className="stack">
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">AGENDA</span><h2>Contas e entradas por dia</h2></div><b>{items.length}</b></div>
      {Object.keys(grouped).length ? <div className="calendar-list">{Object.entries(grouped).map(([day, dayItems]) => <div className="calendar-day" key={day}><div className="day-badge"><b>{day}</b><small>{month.slice(5)}</small></div><div className="day-items">{dayItems.map((item) => <div className={`calendar-entry ${item.paid ? 'paid' : ''}`} key={`${item.id}-${item.date}`}>
        <div><b>{item.title}</b><small>{item.source === 'fixed' ? 'fixo' : item.source === 'card' ? 'cartão' : 'lançamento'}</small></div><strong className={item.kind}>{item.kind === 'income' ? '+' : '-'} {money(item.amount)}</strong>{item.source === 'movement' && <button className="mini" onClick={() => onToggle(item.id.replace('movement-', ''))}>{item.paid ? 'Pago' : 'Marcar'}</button>}
      </div>)}</div></div>)}</div> : <Empty text="Nenhum compromisso para este mês." />}
    </article>
    <article className="info-card"><Bell size={20} /><div><b>Lembretes locais</b><p>Ative em Ajustes para receber avisos antes dos vencimentos. Eles funcionam no aparelho sem depender de servidor.</p></div></article>
  </section>
}
...
function Analysis({ state, month, onCloseMonth }: { state: AppState; month: string; onCloseMonth: () => void }) {
  ...
  return <section className="stack">
    <div className="grid-two"><Metric label="Comprometimento" value={`${commitment.toFixed(1)}%`} hint="despesas / renda garantida" /><Metric label="Resultado projetado" value={money(data[5].guaranteedMargin)} hint="sem renda variável" /></div>
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">EVOLUÇÃO</span><h2>Últimos 6 meses</h2></div></div><div className="chart-bars">{data.map((item) => <div className="chart-column" key={item.month}><div className="chart-value">{money(item.plannedExpenses)}</div><div className="chart-track"><i style={{ height: `${Math.max(5, item.plannedExpenses / maxExpense * 100)}%` }} /></div><small>{item.month.slice(5)}</small></div>)}</div></article>
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">CATEGORIAS</span><h2>Para onde vai o dinheiro</h2></div></div><div className="category-list">{categories.map(([name, value]) => <div key={name}><div><span>{name}</span><b>{money(value)}</b></div><div className="bar"><i style={{ width: `${Math.min(100, value / Math.max(1, categories[0]?.[1] ?? 1) * 100)}%` }} /></div></div>)}</div></article>
    <article className="section-card"><div className="section-heading"><div><span className="section-kicker">FECHAMENTO</span><h2>Planejado x histórico</h2></div>{closing && <span className="tag success">Fechado</span>}</div><p className="muted">Salve um retrato do mês para manter comparações mesmo que você altere seu orçamento no futuro.</p>{closing && <div className="closing-grid"><div><span>Salvo em</span><b>{new Date(closing.closedAt).toLocaleDateString('pt-BR')}</b></div><div><span>Margem</span><b>{money(closing.margin)}</b></div></div>}<button className="secondary full" onClick={onCloseMonth}><RefreshCw size={17} /> {closing ? 'Atualizar fechamento' : 'Fechar este mês'}</button></article>
  </section>
}
...
function Goals({ state, month, setState, onAdd }: { state: AppState; month: string; setState: React.Dispatch<React.SetStateAction<AppState>>; onAdd: () => void }) {
  const addContribution = (goal: Goal) => {
    const raw = window.prompt(`Quanto deseja guardar em ${goal.name}?`)
    const amount = numeric(raw ?? '')
    if (!amount) return
    setState((p) => ({
      ...p,
      goals: p.goals.map((g) => g.id === goal.id ? { ...g, current: Math.min(g.target, g.current + amount) } : g),
      movements: [...p.movements, { id: uid(), description: `Aporte: ${goal.name}`, amount, type: 'expense', category: 'Metas e reserva', date: todayIso(), recurrence: 'none', paid: true }],
    }))
  }
  return <section className="stack">
    <article className="info-card"><PiggyBank size={21} /><div><b>Metas fazem parte do orçamento</b><p>Quando você registra um aporte, ele também entra como saída do mês. Assim a sobra exibida continua real.</p></div></article>
    <button className="primary full" onClick={onAdd}><Plus size={18} /> Criar meta</button>
    {state.goals.map((goal) => { const progress = goal.target > 0 ? Math.min(100, goal.current / goal.target * 100) : 0; const suggested = suggestedGoalContribution(goal, month); return <article className="goal-card" key={goal.id}><div className="goal-top"><div><span className={`priority ${goal.priority}`}>{goal.priority === 'high' ? 'Alta prioridade' : goal.priority === 'medium' ? 'Média prioridade' : 'Baixa prioridade'}</span><h2>{goal.name}</h2></div><button className="icon-button danger" onClick={() => setState((p) => ({ ...p, goals: p.goals.filter((g) => g.id !== goal.id) }))}><Trash2 size={17} /></button></div><strong>{money(goal.current)} <small>de {money(goal.target)}</small></strong><div className="progress large"><i style={{ width: `${progress}%` }} /></div><div className="goal-details"><span>{progress.toFixed(0)}% concluído</span><span>Sugestão mensal: <b>{money(suggested)}</b></span></div>{goal.targetDate && <small className="muted">Objetivo até {new Date(`${goal.targetDate}T12:00:00`).toLocaleDateString('pt-BR')}</small>}<button className="secondary full" disabled={progress >= 100} onClick={() => addContribution(goal)}><PiggyBank size={17} /> Registrar aporte</button></article> })}
  </section>
}

function MoreMenu({ onOpen }: { onOpen: (tab: Tab) => void }) {
  return <section className="menu-grid"><button onClick={() => onOpen('analysis')}><BarChart3 /><div><b>Análises</b><small>Evolução, categorias e fechamento</small></div></button><button onClick={() => onOpen('goals')}><Target /><div><b>Metas e reservas</b><small>Reserva, mudança e objetivos</small></div></button><button onClick={() => onOpen('settings')}><Settings /><div><b>Ajustes</b><small>Orçamento, alertas, segurança e backup</small></div></button></section>
}
...
function MoneyField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="field"><span>{label}</span><div className="money-input"><span>R$</span><input inputMode="decimal" type="number" min="0" step="1" value={Number.isFinite(value) ? value : 0} onChange={(e) => onChange(numeric(e.target.value))} /></div></label>
}

function PercentField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="field"><span>{label}</span><div className="money-input"><span>%</span><input inputMode="decimal" type="number" min="0" max="100" step="0.01" value={Number.isFinite(value) ? value : 0} onChange={(e) => onChange(Math.min(100, numeric(e.target.value)))} /></div></label>
}

function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="field"><span>{label}</span><input inputMode="numeric" type="number" min="0" value={Number.isFinite(value) ? value : 0} onChange={(e) => onChange(Math.round(numeric(e.target.value)))} /></label>
}
...
function DateField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="field"><span>{label}</span><input type="date" value={value} onChange={(e) => onChange(e.target.value)} /></label>
}
